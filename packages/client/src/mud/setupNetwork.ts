...
export async function setupNetwork(networkConfig: NetworkConfig) {
  ...
  const indexerClient = networkConfig.indexerUrl
    ? createIndexerClient({ url: networkConfig.indexerUrl })
    : undefined;
  if (indexerClient) {
    let retriesRemaining = 3;
    while (retriesRemaining > 0) {
      try {
        const seasonPassNamespaceTable = resourceToHex({
          type: "table",
          namespace: "",
          name: "SeasonPassNamesp",
        });
        ...
        if (!("error" in result)) {
          const log = result.ok.logs.find(
            (l) => l.args.tableId === seasonPassNamespaceTable
          );
          if (log) {
            const val = decodeValueArgs(
              { value: "bytes14" },
              log.args as ValueArgs
            ).value;
  ...
  const { components, latestBlock$, storedBlockLogs$ } = await syncToRecs({
    ...
    // large VoxelWorld worlds overloading the RPC
    maxBlockRange: 100n,
  });
